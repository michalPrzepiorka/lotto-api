...
public class LottoDataProvider {
    public static List<LottoData> getLottoData() {
        ...
        List<LottoData> lottoDataList = new LinkedList<>();

        try {
            document = Jsoup.connect("http://megalotto.pl/wyniki/lotto/100-ostatnich-losowan").get();
            ...
            for (Element element : rows) {
                ...
                lottoDataList.add(new LottoData(dateOfTheDraw.text(), lotteryNumbers.text()));
            }

        } catch (IOException e) {
            e.printStackTrace();
        }
        return lottoDataList;
    }
